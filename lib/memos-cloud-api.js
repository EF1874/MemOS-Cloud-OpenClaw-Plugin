...
const MEMOS_USER_ID_TAG_PATTERN = /<memos_userid>([\s\S]*?)<\/memos_userid>/i;
const MEMOS_USER_ID_TAG_PATTERN_GLOBAL = /<memos_userid>[\s\S]*?<\/memos_userid>/gi;
const ENV_SOURCES = [
...
export function buildConfig(pluginConfig = {}) {
  ...
  const allowedAgents = parseAllowedAgents(
  ...
  const useMemosUserIdTag = parseBool(
    cfg.useMemosUserIdTag,
    parseBool(loadEnvVar("MEMOS_USE_MEMOS_USERID_TAG"), false),
  );
  ...
  return {
    ...
    throttleMs,
    _agentOverrides: cfg.agentOverrides ?? {},
  };
}
...
export function extractTaggedMemosUserId(text) {
  if (!text || typeof text !== "string") return "";
  const match = text.match(MEMOS_USER_ID_TAG_PATTERN);
  const value = match?.[1]?.trim();
  return value || "";
}

export function stripTaggedMemosUserId(text) {
  if (!text || typeof text !== "string") return "";
  if (!MEMOS_USER_ID_TAG_PATTERN.test(text)) return text;
  return text
    .replace(MEMOS_USER_ID_TAG_PATTERN_GLOBAL, "")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/\n[ \t]+/g, "\n")
    .replace(/[ \t]+\n/g, "\n");
}
