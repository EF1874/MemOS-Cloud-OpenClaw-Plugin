...
let restartAwaitingDisconnect = false;
...
const UI_TEXT = {
  en: {
    ...
    bannerWaiting: "Config saved. Waiting for the gateway to restart.",
  ...
  zh: {
    ...
    bannerWaiting: "配置已保存，正在等待 gateway 重启。",
...
//   document.title = uiText("heroTitle");
  ...
  elements.restartButton.textContent = uiText("restart");
...
function renderForm() {
  ...
  elements.restartButton.disabled = !remoteState || restartPending;
...
function refreshChrome() {
  ...
  if (restartPending) {
    const waitingTooLong = restartAwaitingDisconnect && restartRequestedAt > 0 && Date.now() - restartRequestedAt > 3500;
    setBanner("info", waitingTooLong ? uiText("bannerRestartRefreshHint") : uiText("bannerWaiting"));
    return;
  }
  if (isDirty()) {
...
async function loadRemote(initial = false) {
  ...
  try {
    ...
    if (restartPending) {
      if (restartAwaitingDisconnect) {
        const waitingTooLong = restartRequestedAt > 0 && Date.now() - restartRequestedAt > 3500;
        if (waitingTooLong) {
          restartPending = false;
          restartAwaitingDisconnect = false;
          restartRequestedAt = 0;
          setOverlay(false, "", "");
          setBanner("info", uiText("bannerRestartRefreshHint"));
          refreshChrome();
          return;
        }
        setOverlay(false, "", "");
        setBanner("info", uiText("bannerWaitingStop"));
        refreshChrome();
        return;
      }
      ...
      renderForm();
      setOverlay(false, "", "");
      setBanner("info", uiText("bannerRestarted"));
    } else {
      refreshChrome();
    }
  } catch (error) {
    if (restartPending) {
      try {
        ...
        restartRequestedAt = 0;
        setOverlay(false, "", "");
        setBanner("info", uiText("bannerHeartbeatRecovered"));
        setTimeout(() => {
          window.location.reload();
        }, 350);
        return;
        ...
        restartAwaitingDisconnect = false;
        setOverlay(false, "", "");
        setBanner("info", uiText("bannerWaitingBack"));
      }
      return;
    }
    if (error?.status === 403) {
...
async function restartGateway() {
  try {
    ...
    setOverlay(true, uiText("overlayTitle"), uiText("overlayText"));
    setBanner("info", uiText("bannerRestartLaunched"));
  } catch (error) {
    restartPending = false;
    restartAwaitingDisconnect = false;
    restartRequestedAt = 0;
...
elements.restartButton.addEventListener("click", () => void restartGateway());
